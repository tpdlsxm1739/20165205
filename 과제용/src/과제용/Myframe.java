...
public class Myframe {
	...
	public static void main(String[] args) {
		playerName = JOptionPane.showInputDialog("도둑의 이름을 입력하시오:");
		new Myframe();
	}
		...
		public void actionPerformed(ActionEvent e) {
			...
			for (PosImageIcon s : moneyList) {
				if (s.collide(new Point(player.x, player.y))) {
					...
					s.x = (int) (Math.random() * 650 + 1);
					s.y = (int) (Math.random() * 650 + 1);
	...
	private PosImageIcon getRandomAttacker(String pic, int margin, double steps) {
		...
		public void actionPerformed(ActionEvent event) {
			...
			if (times % DRINK_INTERVAL == 0) {
				if (drinkList.isEmpty()) // 현재 활동 중이 아니면 하나 추가
					drinkList.add(getRandomAttacker(DRINK, BOY_MARGIN, policeStep));
			...
			if (times % POLICE_INTERVAL == 0) {
				policeList.add(getRandomAttacker(POLICE, POLICE_MARGIN, policeStep));
			}
		}
	}
